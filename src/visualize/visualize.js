...
export function applyTransform() {
  ...
  // This makes the background grid move with the workspace
  // runtime.container.style.backgroundPosition = `${x}px ${y}px`;
...
export function visualize(data) {
  ...
  function walk(obj, label, x) {
    ...
    // 1. Calculate REAL height based on text content
    // Header (40px) + (Rows * 24px) + Padding (20px)
    ...
    return {
      ...
      render: (finalY) => {
        // Center node vertically within its branch
  ...
  // 3. IMPROVED CENTERING: Focus on the center of the WHOLE tree
